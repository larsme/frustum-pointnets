...
def get_instance_seg_v1_net(point_cloud, one_hot_vec, is_training, bn_decay):
    ...
    Input:
        point_cloud: TF tensor in shape (B,N,3(+1)(+3))
    ...
    Output:
        ...
        end_points: dict
    '''
    with tf.variable_scope("instance_seg_v1_net"):
        ...
        pc_pred_logits = tf_util.conv2d(net, 2, [1,1],
                             padding='VALID', stride=[1,1], activation_fn=None,
                             scope='conv10')
        pc_pred_logits = tf.squeeze(pc_pred_logits, [2], 'pc_pred_logits') # BxNxC
    # return logits,
...
def get_model(pc_in_box, box_label_prob, is_training, bn_decay=None):
    ...
    # pc_pred_logits, end_points = get_instance_seg_v1_net(\
    #     pc_in_box, box_label_prob,
    #     is_training, bn_decay, end_points)
    # end_points['mask_logits'] = logits

    pc_pred_logits = get_instance_seg_v1_net(pc_in_box, box_label_prob, is_training, bn_decay)

    # # Masking
    # # select masked points and translate to masked points' centroid
    # object_point_cloud_xyz, mask_xyz_mean, end_points = \
    #     point_cloud_masking(pc_in_box, logits, end_points)
    #
...
if __name__=='__main__':
    with tf.Graph().as_default():
        ...
        outputs = get_model(inputs, tf.ones((32, 3), tf.float32), tf.constant(True))
        for key in outputs:
            print((key, outputs[key]))
        loss = get_loss(tf.zeros((32, 1024), dtype=tf.int32), tf.zeros((32, 1024), dtype=tf.int32))
        print(loss)
