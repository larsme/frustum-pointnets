...
from model_util import placeholder_inputs, get_loss


# def get_instance_seg_v2_net(point_cloud, one_hot_vec,
#                             is_training, bn_decay, end_points):
def get_instance_seg_v2_net(pc_in_box, box_label_prob, is_training, bn_decay):
    ''' 3D instance segmentation PointNet v2 network.
    Input:
        pc_in_box: TF tensor in shape (B,N,3(+1)(+3))
            frustum point clouds with XYZ (and intensity) (and colors) in point channels
            XYZs are in frustum coordinate
        box_label_prob: TF tensor in shape (B,3)
            length-3 vectors indicating predicted object type
        is_training: TF boolean scalar
        bn_decay: TF float scalar
        end_points: dict
    Output:
        pc_pred_logits: TF tensor in shape (B,N,2), scores for bkg/clutter and object
        end_points: dict
    '''

    with tf.variable_scope("instance_seg_v2_net"):
        l0_xyz = tf.slice(pc_in_box, [0, 0, 0], [-1, -1, 3])
        l0_points = tf.slice(pc_in_box, [0, 0, 3], [-1, -1, 1])
        ...
        l3_points = tf.concat([l3_points, tf.expand_dims(box_label_prob, 1)], axis=2)
        ...
        pc_pred_logits = tf_util.conv1d(net, 2, 1,
            padding='VALID', activation_fn=None, scope='conv1d-fc2')

    # return pc_pred_logits, end_points
    return pc_pred_logits
...
def get_model(pc_in_box, box_label_prob, is_training, bn_decay=None):
    ...
    # pc_pred_logits, end_points = get_instance_seg_v2_net(\
    #     pc_in_box, box_label_prob,
    #     is_training, bn_decay, end_points)
    # end_points['mask_logits'] = pc_pred_logits
    pc_pred_logits = get_instance_seg_v2_net(pc_in_box, box_label_prob, is_training, bn_decay)

    # # Masking
    # # select masked points and translate to masked points' centroid
    # object_point_cloud_xyz, mask_xyz_mean, end_points = \
    #     point_cloud_masking(pc_in_box, pc_pred_logits, end_points)
    #
    # # T-Net and coordinate translation
    # center_delta, end_points = get_center_regression_net(\
    #     object_point_cloud_xyz, box_label_prob,
    #     is_training, bn_decay, end_points)
    ...
    #     object_point_cloud_xyz_new, box_label_prob,
    #     is_training, bn_decay, end_points)
    #
